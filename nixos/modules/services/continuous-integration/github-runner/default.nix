  ...
  options.services.github-runner = {
    ...
    ephemeral = mkOption {
      ...
      description = ''
        ...
        <itemizedlist>
          <listitem><para>Passes the <literal>--ephemeral</literal> flag to the runner configuration script</para></listitem>
          <listitem><para>De-registers and stops the runner with GitHub after it has processed one job</para></listitem>
          <listitem><para>On stop, systemd wipes the runtime directory (this always happens, even without using the ephemeral option)</para></listitem>
          <listitem><para>Restarts the service after its successful exit</para></listitem>
          <listitem><para>On start, wipes the state directory and configures a new runner</para></listitem>
        </itemizedlist>

        You should only enable this option if <literal>tokenFile</literal> points to a file which contains a
